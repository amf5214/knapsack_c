...
void instantiateItemset(char*** labels, int** values, int** weights, unsigned long size) {
    printf("size provided for instantiation = %lu\n", size);
...
*/
int calculateRoute(Hashmap* map, char* route, int* weights, int* values, Path** solutions, unsigned long* solutionsCount) {
    if(strlen(route) == 0) {
        return 0;
    }
...
*/
Path** findValidRoutes(char** labels, int* weights, int* values, unsigned long size, unsigned long* solutionsCount) {
    Path** solutions = (Path**) malloc(sizeof(Path*) * pow(2, size));

    Hashmap* map;
    unsigned long minvalue = 0;
    if(pow(2, size + 1) > ULONG_MAX) {
        minvalue = ULONG_MAX;
    } else {
	minvalue = pow(2, size + 1);
    }

    instantiateHashmap(&map, minvalue);

    unsigned long currentPlace = 0;
    ...
    queue->head = startingNode;
    queue->tail = startingNode;

    while(currentPlace < size + 1) {
        char** exploredPaths = (char**) malloc(sizeof(char*) * minvalue);
        int numPaths = 0;
        ...
        while(queue->head != NULL) {
            ...
            if(weight <= MAX_SOLUTION_WEIGHT) {
                exploredPaths[numPaths] = (char*) malloc(sizeof(char) * 255);
        ...
        if(currentPlace < size) {
            for(int i = 0; i < numPaths; i++) {
                if(strlen(exploredPaths[i]) > 0) {
                    char* expandedPath = (char*) malloc(sizeof(char) * 255);
                    strcpy(expandedPath, "");
		    strcat(expandedPath, exploredPaths[i]);
                    strcat(expandedPath, "-");
                    char* value = (char*) malloc(sizeof(char) * 255);
                    sprintf(value, "%lu", currentPlace); 
                    strcat(expandedPath, value);
                    free(value);

                    Node* node1 = (Node*) malloc(sizeof(Node));
                    node1->name = (char*) malloc(sizeof(char) * 255);
                    strcpy(node1->name, exploredPaths[i]);

                    Node* node2 = (Node*) malloc(sizeof(Node));
                    node2->name = (char*) malloc(sizeof(char) * 255);
...
int main(int argc, char *argv[]) {
    ...
    unsigned long count = countLines(file);

    printf("lines in code = %lu\n", count);
    ...
    unsigned long solutionBase = 0;
    unsigned long* solutionCount = &solutionBase;

    Path** solutions = findValidRoutes(labels, wt, val, count, solutionCount);

    printf("solutions = %lu\n", *solutionCount);

    for(unsigned long i = 0; i < *solutionCount; i++) {
        printf("[solution %lu = {%s}, weight = {%d}, value = {%d}]\n", i, (*(solutions[i])).name, (*(solutions[i])).weight, (*(solutions[i])).value);
    }
